...
for (const t of spec.teammates) {
  ...
  if ((t.skills ?? []).length > 0) {
    y = y.replace(/(- id: skill-filesystem\n  name: '@deepseek-ai\/dsh-skill-filesystem'\n)/,
...
// The modes doc is generated too, so it cannot drift from the roster.
writeFileSync(resolve(presetsDir, 'DESK-BOTS.md'), `# webfaCe Desk modes
...
|---|---|---|---|
${spec.teammates.map(t => `| \`${t.id}\` | ${t.name} | ${t.shell ? 'yes' : 'no'} | ${(t.skills ?? []).join(', ')} |`).join('\n')}
