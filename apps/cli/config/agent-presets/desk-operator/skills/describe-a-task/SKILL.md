...
2. Create it with `schedule_create` (if that tool is not in this conversation — it is only there for top-level modes — ask the owner to say it again in a new Desk conversation): use `every_seconds` for repeating routines (minimum 300), `at` for one-offs. The prompt you store must be self-contained — the future run does not see this conversation.
