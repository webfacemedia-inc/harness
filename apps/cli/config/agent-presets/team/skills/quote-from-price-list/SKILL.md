...
4. If the owner wants a document: call `mcp__kit__make_pdf` (title "Quote — <customer>", the quote as Markdown with a table of items, the total, and a validity line) — it lands on the business letterhead; add `mcp__kit__make_document` when they want Word too. Reply with a two-line summary (total, validity) and the 📄 link line the tool returns — not the whole quote. Never write the file yourself.
