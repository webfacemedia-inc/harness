...
const link = p => { const rel = p.startsWith(WORK) ? p.slice(WORK.length + 1) : p; return { path: p, url: `/files/dl/${rel.split('/').map(encodeURIComponent).join('/')}`, name: rel.split('/').pop() } }
const done = (files, note = '') => { const items = files.map(link); return text(`${note ? note + '\n' : ''}Saved:\n${items.map(i => `- [${i.name}](${i.url})`).join('\n')}\nTell the owner with the link(s) above; the file is also under Files → deliverables.`) }
