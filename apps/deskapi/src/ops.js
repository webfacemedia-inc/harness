...
<td><span class="pill ${o.status === 'ready' ? '' : 'off'}">${esc(o.status)}</span>${o.usage ? `<br><small>${Math.round((o.usage.monthTokens ?? 0) / 1000)}k tokens this month · ${o.usage.sessions ?? 0} sessions</small>` : ''}${o.lastError ? `<br><small style="color:#b42318" title="${esc(o.lastError.at ?? '')}">${esc(o.lastError.step)}: ${esc(o.lastError.message)}</small>` : ''}${(o.attempts ?? 0) > 1 ? `<br><small>attempt ${o.attempts}</small>` : ''}${o.billing && o.billing !== 'ok' ? `<br><span class="pill warn">${esc(o.billing)}</span>` : ''}<br><small>beat ${when(o.lastHeartbeat)}</small><br><small>snap ${when(o.lastSnapshot)}</small></td>
...
.card{width:100%;max-width:400px;background:#fff;border:1px solid var(--line);border-radius:16px;padding:32px}.brand{display:flex;align-items:center;gap:10px;font-size:20px;font-weight:600;margin-bottom:6px}.brand .wm em{font-style:normal;font-family:Fraunces,Georgia,serif;font-weight:600;font-size:1.1em;color:var(--blue-deep,#1f6f99);margin-left:.22em}h1{font-family:Fraunces,serif;font-weight:600;font-size:26px;margin:14px 0 6px}p{color:var(--mute);margin:0 0 18px;font-size:15px}
...
<body><form class="card" method="post" action="/ops/login"><div class="brand"><span class="wm">webfaCe<em>Desk</em></span></div><h1>Operator console</h1><p>Every Desk this store runs — create, pause, resend, destroy. Operators only.</p><label for="k">Ops key</label><input id="k" name="key" type="password" autocomplete="current-password" autofocus required><button type="submit">Open the console</button></form></body></html>`)
