...
export function layout({ title, business, body, back = '/', backLabel = 'Back to Desk', head = '' }) {
  return `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)} · webfaCe Desk</title><script>/* Downloads are plain navigations to an attachment response — the same thing the Session log export does — so the desktop app's own save dialog handles them. */function deskDownload(u){fetch(u,{method:'HEAD',credentials:'same-origin'}).catch(function(){}).then(function(){location.assign(u)});return false}</script><script src="https://insights.webfacemedia.com/api/script.js" data-site="webface" defer></script>
