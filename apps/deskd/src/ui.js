...
export function layout({ title, business, body, back = '/', backLabel = 'Back to Desk', head = '' }) {
  return `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)} · webfaCe Desk</title><script>/* Download strategies, in order: the desktop app's runtime (opens the system browser, which saves natively), the app's message bridge, then a scripted <a download> (what the Session log export does). The bar shows which one ran. */
function deskDownload(u,name){var abs=new URL(u,location.href).href,st=document.getElementById('dlstatus'),say=function(m){if(st)st.textContent=m};
if(window.runtime&&typeof window.runtime.BrowserOpenURL==='function'){window.runtime.BrowserOpenURL(abs);say('sent to your browser (app runtime)');return false}
var h=window.webkit&&window.webkit.messageHandlers&&window.webkit.messageHandlers.external;if(h){try{h.postMessage('BO:'+abs);say('sent to your browser (app bridge)');return false}catch(e){}}
var a=document.createElement('a');a.href=abs;a.download=name||abs.split('/').pop().split('?')[0];a.style.display='none';document.body.appendChild(a);a.click();setTimeout(function(){a.remove()},1000);say('download started (browser)');return false}</script><script src="https://insights.webfacemedia.com/api/script.js" data-site="webface" defer></script>
