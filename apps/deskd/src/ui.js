...
export function layout({ title, business, body, back = '/', backLabel = 'Back to Desk', head = '' }) {
  return `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)} · webfaCe Desk</title><script>/* Downloads work exactly like the Session log export: a temporary <a download> clicked by script — the download attribute is what makes an embedded WebView save instead of display. */function deskDownload(u,name){var a=document.createElement('a');a.href=u;a.download=name||u.split('/').pop().split('?')[0];a.style.display='none';document.body.appendChild(a);a.click();setTimeout(function(){a.remove()},1000);return false}</script><script src="https://insights.webfacemedia.com/api/script.js" data-site="webface" defer></script>
