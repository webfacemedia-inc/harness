...
export async function handle(req, res, u, { business }) {
  ...
  if (p === '/files/view') {
    ...
    const body = `<div class="bar"><a class="btn ghost" href="/" style="margin:0;padding:7px 12px;font-size:13px">← Back to Desk</a><span class="h" style="margin-left:12px;font-size:13px;color:var(--mute)">${esc(rel.split('/').pop())}</span><a class="btn" href="${dl}" download onclick="return deskDownload(this.href)" style="margin:0 0 0 auto;padding:7px 12px;font-size:13px">Download</a></div>
${kind === 'image' ? `<div class="view"><img src="${dl}?inline=1" alt=""></div>` : kind === 'frame' ? `<iframe class="view" src="${dl}?inline=1" title="${esc(rel)}"></iframe>` : `<div class="view none"><p>This file type has no preview here. <a href="${dl}" download onclick="return deskDownload(this.href)">Download it</a> and open it on your computer.</p></div>`}`
