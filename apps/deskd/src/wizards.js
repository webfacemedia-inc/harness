...
function shell(title, business, inner) {
...
main{max-width:640px;margin:0 auto;padding:22px 20px 60px}
...
.eyebrow{font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:var(--mute);margin:0 0 6px}h1{font-size:24px;margin:0 0 8px}p{margin:0 0 12px}p.mute{color:var(--mute)}
.card{background:var(--card);border:1px solid var(--line);border-radius:14px;padding:22px}
.btn,button{display:inline-block;padding:11px 16px;border:0;border-radius:8px;background:var(--blue);color:#fff;font:inherit;font-weight:600;cursor:pointer;text-decoration:none;margin:6px 8px 6px 0}.btn:hover,button:hover{background:var(--blue-ink)}
.ghost{background:transparent;color:var(--blue-ink);border:1px solid var(--line)}
.copy{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin:10px 0}.copy code{flex:1 1 auto;min-width:0;overflow-wrap:anywhere;font-size:13px;background:rgba(127,127,127,.12);padding:8px 10px;border-radius:8px}
label{display:block;font-weight:600;font-size:13px;margin:12px 0 6px}input,textarea{width:100%;padding:10px 12px;border:1px solid var(--line);border-radius:8px;background:transparent;color:inherit;font:inherit}textarea{min-height:120px;font-family:ui-monospace,Menlo,monospace;font-size:13px}
.msg{padding:10px 12px;border-radius:8px;margin:0 0 12px;font-size:14px}.ok{background:rgba(31,138,91,.1);color:var(--ok)}.err{background:rgba(180,35,24,.08);color:var(--err)}
.nav{display:flex;justify-content:space-between;align-items:center;margin-top:16px;gap:8px;flex-wrap:wrap}
img.shot{max-width:100%;border:1px solid var(--line);border-radius:10px;margin:8px 0}
</style><body><div class="top"><a href="/connections">← Connections</a><span style="color:var(--mute);font-size:13px">${esc(business)}</span></div><main>
<script>function cp(t,b){navigator.clipboard.writeText(t).then(()=>{b.textContent='Copied';setTimeout(()=>b.textContent='Copy',1500)})}</script>${inner}</main></body></html>`
}
...
export function googleStep({ business, host, step, msg, err, google, cfg }) {
  ...
  const S = {
    1: ['Create a Google Cloud project', `<p>Google needs a project that belongs to <strong>${esc(business)}</strong>. Name it after the business and press <em>Create</em>. It takes about ten seconds to appear.</p><p class="mute">Use the Google account the business runs on (the one whose email Desk should read).</p><a class="btn" href="https://console.cloud.google.com/projectcreate" target="_blank" rel="noopener">Open “Create project” ↗</a>`],
    2: ['Turn on the four Google APIs', `<p>Press <em>Enable</em> on each of these. Each opens in a new tab; come back here after each one.</p>${['gmail.googleapis.com|Gmail', 'calendar-json.googleapis.com|Calendar', 'drive.googleapis.com|Drive', 'people.googleapis.com|People (contacts)'].map(x => { const [api, l] = x.split('|'); return `<a class="btn ghost" href="https://console.cloud.google.com/apis/library/${api}${q}" target="_blank" rel="noopener">Enable ${l} ↗</a>` }).join('')}`],
    3: ['Set up the consent screen', `<p>Open <em>Google Auth Platform</em> → <em>Get started</em>. App name: <strong>${esc(business)}</strong>. Support and contact email: yours. Audience: <strong>External</strong>. Finish the short form.</p><a class="btn" href="https://console.cloud.google.com/auth/overview${q}" target="_blank" rel="noopener">Open Google Auth Platform ↗</a>`],
    4: ['Add yourself as a test user', `<p>Until Google reviews the app, only listed “test users” can sign in. Under <em>Audience</em> → <em>Test users</em> add the Google address you will connect (the business mailbox).</p><a class="btn" href="https://console.cloud.google.com/auth/audience${q}" target="_blank" rel="noopener">Open Audience ↗</a>`],
    5: ['Create the client', `<p><em>Clients</em> → <em>Create client</em>. Type: <strong>Web application</strong>. Name: <strong>Desk</strong>. Under <em>Authorised redirect URIs</em> press <em>Add URI</em> and paste exactly:</p>${copyRow(uri)}<p>Press <em>Create</em>, then <em>Download JSON</em> on the new client.</p><a class="btn" href="https://console.cloud.google.com/auth/clients/create${q}" target="_blank" rel="noopener">Open “Create client” ↗</a>`],
  ...
  return shell('Set up Google', business, `${bar(step, 7, done)}<p class="eyebrow">Google · step ${step} of 7</p><h1>${title}</h1>${msg ? `<div class="msg ok">${esc(msg)}</div>` : ''}${err ? `<div class="msg err">${esc(err)}</div>` : ''}<div class="card">${body}</div><div class="nav">${back(step)}${step < 6 ? next(step) : step === 6 ? (google.clientConfigured ? `<a class="btn" href="/connections/google/setup?step=7">Next</a>` : '<span></span>') : ''}</div>`)
}
...
export function wpStep({ business, step, msg, err, state }) {
  ...
  const [title, body] = S[step]
  return shell('Connect WordPress', business, `${bar(step, 3, [])}<p class="eyebrow">WordPress · step ${step} of 3</p><h1>${title}</h1>${msg ? `<div class="msg ok">${esc(msg)}</div>` : ''}${err ? `<div class="msg err">${esc(err)}</div>` : ''}<div class="card">${body}</div>`)
}
