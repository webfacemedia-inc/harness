...
export function googleStep({ business, host, step, msg, err, google, cfg }) {
  ...
  const S = {
    1: ['Create a Google Cloud project', `<p>Google needs a project that belongs to <strong>${esc(business)}</strong>. Name it after the business and press <em>Create</em>. It takes about ten seconds to appear.</p><p class="mute">Use the Google account the business runs on (the one whose email Desk should read). Tip: Google asks for your passkey less often if you do these steps on a laptop where you are already signed in.</p><a class="btn" href="https://console.cloud.google.com/projectcreate" target="desk-console" rel="noopener">Open “Create project”${ICONS.ext}</a>`],
    2: ['Turn on the four Google APIs', `<p>One press enables Gmail, Calendar, Drive and People (contacts) together. Google may ask you to verify with your passkey once.</p><a class="btn" href="https://console.cloud.google.com/flows/enableapi?apiid=gmail.googleapis.com,calendar-json.googleapis.com,drive.googleapis.com,people.googleapis.com${pid ? '&project=' + encodeURIComponent(pid) : ''}" target="desk-console" rel="noopener">Enable all four APIs${ICONS.ext}</a><details><summary style="font-size:13px">Enable them one at a time instead</summary>${['gmail.googleapis.com|Gmail', 'calendar-json.googleapis.com|Calendar', 'drive.googleapis.com|Drive', 'people.googleapis.com|People (contacts)'].map(x => { const [api, l] = x.split('|'); return `<a class="btn ghost" href="https://console.cloud.google.com/apis/library/${api}${q}" target="desk-console" rel="noopener">Enable ${l}${ICONS.ext}</a>` }).join('')}</details>`],
