...
Desktop app: apps/desktop (Wails v2, vendored at apps/desktop/third_party/wails with a WKDownload delegate) — a window onto a Desk; releases via .github/workflows/desktop.yml + apps/desktop/scripts/release-mac.sh (sign + notarise + DMG). Since v0.1.9 the app self-updates on macOS (apps/desktop/selfupdate_darwin.go: release zip → codesign + TeamIdentifier pin → bundle swap → relaunch; dev/translocated/unwritable copies fall back to /download). Live checks: DESK_UPDATE_LIVE_TEST=1 go test in apps/desktop.
```
